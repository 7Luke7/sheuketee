...
const Account = () => {
  ...
  const showResetHandler = async () => {
    if (!user().email)
      return alert(
        "ამჟამად ვერიფიკაცია მხოლოდ მეილით არის შესაძლებელი, გთხოვთ დაამატოთ მეილი."
      );
    try {
      setShowReset(true);
      ...
      <Switch>
        <Match when={!showReset()}>
          <div class="flex-[6] flex flex-col h-full justify-between border-l pl-12">
            {user() && !user().email && (
              <p class="font-[thin-font] text-sm font-bold text-red-500">
                პაროლის შეცვლა ხდება მხოლოდ მეილის გამოყენებით გთხოვთ დაამატოთ
                მეილი.
              </p>
            )}
            ...
            <div>
              <div class="flex mt-5 gap-x-2 items-center">
                <img
                  class="rounded-[50%] bg-gray-100"
                  src={exclamationSVG}
                ></img>
                <h1 class="font-[thin-font] text-gr text-xs font-bold">
                  კოდი იგზავნება მხოლოდ მეილზე.
                </h1>
              </div>
              <button
                onClick={showResetHandler}
                class="text-white mt-2 px-6 rounded-[16px] py-2 text-base font-[thin-font] bg-gray-900 hover:bg-gray-800 font-bold duration-200 ease-in"
              >
                პაროლის შეცვლა
              </button>
            </div>
        ...
        <Match when={showReset() && !showPasswordForm()}>
          <div class="flex-[6] flex flex-col justify-between h-full border-l pl-12">
            <form method="post" onSubmit={handleFormSubmit}>
              ...
              <input
                class="bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-2 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                id="verif"
                type="text"
                name="verify"
                placeholder="4 ციფრა კოდი"
              />
              <div>
                <button
                  type="submit"
                  class="text-white flex items-center justify-center gap-x-2 px-4 py-2 text-base font-[thin-font] font-bold bg-dark-green hover:bg-dark-green-hover rounded-[16px]"
                >
                  გაგრძელება
                </button>
              </div>
            ...
            <Show when={timer()}>
              <div class="flex flex-col gap-y-1">
                <p class="font-[thin-font] text-gr text-sm font-bold">
                  ვერფიკაციის კოდი გამოიგზავნა მეილზე.
                </p>
                <p class="font-[thin-font] text-gr text-sm font-bold">
                  კოდის არა ეფექტურობამდე დარჩენილია {timer()} წამი.
                </p>
              </div>
            </Show>
            {error() && (
              <p class="font-[thin-font] text-gr text-sm font-bold">
                ვერფიკაციის კოდი უკვე გამოგზავნილია მეილზე გთხოვთ მოიცადოთ 3
                წუთი.
              </p>
            )}
          </div>
        ...
        <Match when={showPasswordForm()}>
          <form
            ...
            class="flex-[6] border-l pl-12 flex-col h-full flex justify-between"
          >
            <div class="flex flex-col w-[300px] gap-y-1">
              <div class="flex items-center focus:outline-none border border-gray-200 rounded py-3 px-4 mb-2 bg-gray-200 focus:bg-white focus:border-gray-500 justify-between">
                ...
                />
                {displayNewPassword() ? (
                  <button onClick={() => setDisplayNewPassword(false)}>
                    <img src={eyeSlashSVG}></img>
                  </button>
                ) : (
                  <button onClick={() => setDisplayNewPassword(true)}>
                    <img src={eyeFillSVG}></img>
                  </button>
                )}
              </div>
              <div class="flex items-center bg-gray-200 border border-gray-200 rounded py-3 px-4 mb-2 focus:outline-none focus:bg-white focus:border-gray-500 justify-between">
                ...
                {displayRepeatPassword() ? (
                  <button onClick={() => setDisplayRepeatPassword(false)}>
                    <img src={eyeSlashSVG}></img>
                  </button>
                ) : (
                  <button onClick={() => setDisplayRepeatPassword(true)}>
                    <img src={eyeFillSVG}></img>
                  </button>
                )}
              </div>
