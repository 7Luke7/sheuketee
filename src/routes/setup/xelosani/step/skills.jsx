...
const Skills = () => {
  ...
  const handleBodyClick = (event) => {
    if (event.target.id === "job_scroll" || event.target.id === "job-btn") {
      ...
      if (
        document.getElementById("job_scroll") &&
      ...
      <Switch>
        <Match when={check_jobs() !== 200 && !submitted()}>
              ...
              <div class="flex mb-3 items-center gap-2 flex-wrap">
                <For each={selectedJobs()}>
                  {(sj) => (
                    <button
                      onClick={() =>
                        setSelectedJobs((prev) => prev.filter((a) => a !== sj))
                      }
                      class="flex px-2 py-1 gap-x-1 items-center bg-gray-100 rounded-[16px]"
                    >
                      <img
                        width={16}
                        class="border-2 border-gr rounded-full"
                        height={16}
                        src={clear}
                      ></img>
                      <p class="font-[thin-font] font-bold text-gray-700">
                        {sj}
                      </p>
                    </button>
                  )}
          ...
          <div class="h-full w-full relative">
            <input
              onInput={(e) => job_search(e.target.value.trim())}
              autocomplete="off"
              id="input"
              onClick={() => {
              ...
              class="bg-gray-200 border border-gray-200 rounded appearance-none w-full bg-transparent outline-none block text-gray-700 font-bold font-[thin-font] leading-tight py-3 px-4"
              type="text"
              name="skill"
              placeholder="მოძებნე უნარი"
            />
            <Show when={allJobs()}>
              <div
                id="job_scroll"
                class="border custom-scrollbar absolute bg-white h-[200px] overflow-y-scroll border-t-0 border-gray-200 w-full"
              >
                <Switch>
                  <Match when={allJobs()}>
                    <For each={allJobs()}>
                      {(j, i) => {
                        return (
                          <button
                            ...
                            id="job-btn"
                            class="w-full text-left cursor-pointer text-lg px-3 py-2 hover:bg-gray-100 duration-200 font-[thin-font] font-bold my-1"
                          >
                            {j}
                          </button>
                        );
            ...
            <button
              ...
              className="py-2 w-full mt-3 px-3 rounded-md text-sm font-[thin-font] font-bold bg-dark-green text-white transition-all duration-500 hover:bg-dark-green-hover"
            >
              გაგრძელება
            </button>
          </div>
