    ...
    const fetch_location = async (lng, lat) => {
        try {
            ...
            if (response.ok) {
                ...
                return setMessage('დაფიქსირდა შეცდომა: თქვენს მიერ მარკირებული ადგილი ვერ მოიძებნა, ცადეთ თავიდან.');
            }
        } catch(error) {
        ...
        <Switch>
                ...
                <div class="flex h-full flex-col items-start justify-start">
                    <div class="h-full border-b w-full m-0">
                        <div class="h-full w-full m-0 relative" ref={map}>
                            <div class={`absolute max-w-[280px] ${isSearchOpen() && "h-full opacity-[90%] overflow-y-auto"} right-0 z-50 bg-white`}>
                                <Switch>
                                    <Match when={isSearchOpen()}>
                                        <div class="flex gap-x-2 items-center py-2 px-3 border-b">
                                            <form onSubmit={handleSearchLocation} class="flex w-full justify-between items-center">
                                                <input value={searchInputValue()} onChange={(e) => setSearchInputValue(e.target.value)} autocomplete="off" type="text" name="searchLocation" class="w-full outline-none text-gray-800 font-[thin-font] font-bold" placeholder="მოძებნე..." />
                                                <button type="submit">
                                        ...
                                        <div class="py-2 px-3">
                                            <Switch>
                                                <Match when={searching()}>
                                                    <p class="text-gray-800 text-center font-[thin-font] font-bold">იტვირთება...</p>
                                                </Match>
                                                <Match when={!searching() && !searchResults()}>
                                                    <p class="text-gray-800 text-center font-[thin-font] font-bold">მისამართები ვერ მოიძებნა.</p>
                                                </Match>
                                                <Match when={!searching() && searchResults()}>
                                                    <ul>
                                                        {searchResults().map((result, index) => (
                                                            <li onClick={() => handleSidebarLocationChange(result)} key={index} class={`text-xs font-bold font-[thin-font] break-word p-1 cursor-pointer ${markedLocation()?.place_id === result.place_id ? "bg-gray-200" : "hover:bg-gray-100"} border-b`}>{result.display_name}</li>
                                                        ))}
                                                    </ul>
                                                </Match>
                                            </Switch>
                                    ...
                                    <Match when={!isSearchOpen()}>
                                        <button onClick={() => setIsSearchOpen(true)} class="flex items-center justify-center p-2">
                                            <img src={SearchIcon} alt="Search" />
                                        </button>
                                    </Match>
                                </Switch>
                    ...
                    <div class="px-2 w-full flex flex-col gap-y-2 my-2">
                        ...
                        <div class="flex items-center gap-x-1">
                            ...
                            <p class="text-[12px] text-gray-800 font-[thin-font] break-word font-bold">{markedLocation()?.display_name}</p>
                        </div>
                        <button onClick={handleLocationSubmit} className="py-2 w-full px-3 rounded-md text-sm font-[thin-font] font-bold bg-dark-green text-white transition-all duration-500 hover:bg-dark-green-hover">
                            გაგრძელება
                        </button>
                    </div>
                </div>
            ...
            <Match when={location() !== 200 || submitted()}>
                <div class="flex w-full flex-col justify-center h-full items-center">
                    ...
                    <A className="py-2 mt-3 w-1/2 text-center rounded-md text-sm font-[thin-font] font-bold bg-dark-green text-white transition-all duration-500 hover:bg-dark-green-hover" href="/setup/damkveti/step/about">გაგრძელება</A>
                </div>
            </Match>
        </Switch>
    </div>
